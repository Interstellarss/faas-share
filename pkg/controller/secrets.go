...
func UpdateSecrets(sharepod *faasv1.SharePod, deployment *appsv1.Deployment, existingSecrets map[string]*corev1.Secret) error {
	// Add / reference pre-existing secrets within Kubernetes
	secretVolumeProjections := []corev1.VolumeProjection{}

	//how to set secrets in sharepod?
	for _, secretName := range sharepod.Spec.Secrets {
	...
	volumeName := fmt.Sprintf("%s-projected-secrets", sharepod.Name)
