...
type SharePodFactory struct {
	Factory k8s.FunctionFactory
}

func NewFunctionFactory(clientset kubernetes.Interface, config k8s.DeploymentConfig) SharePodFactory {
	return SharePodFactory{
		k8s.FunctionFactory{
			Client: clientset,
			Config: config,
		},
...
func (f *SharePodFactory) MakeProbes(sharepod *faasv1.SharePod) (*k8s.SharepodProbes, error) {
...
func (f *SharePodFactory) ConfigureContainerUserID(deployment *appsv1.Deployment) {
